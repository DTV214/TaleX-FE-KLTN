...
import { CreditCard, LayoutDashboard, LogOut, Megaphone, BarChart2, PlusCircle } from "lucide-react";
import { SidebarLabelPopover } from './sidebar-label-popover';
    ...
    <div className="flex h-screen w-full bg-[#F8F8F8] text-[#161823]">
      ...
      <div className="w-16 shrink-0 relative z-50 h-full">
        ...
        <div className="group absolute top-0 left-0 flex h-full flex-col border-r border-slate-200 bg-white transition-all duration-300 w-16 hover:w-64 hover:shadow-xl overflow-hidden">
          ...
          <nav className="flex flex-1 flex-col gap-1 overflow-y-auto overflow-x-hidden py-4 text-sm font-medium">
            ...
            <div className="px-2">
              <div onClick={handleLogout} className="flex items-center gap-3 rounded-md p-2 text-slate-500 hover:bg-slate-100 hover:text-[#161823] cursor-pointer mt-4">
                <LogOut className="h-5 w-5 shrink-0" />
                <span className="opacity-0 transition-opacity duration-300 group-hover:opacity-100 whitespace-nowrap">Account settings</span>
              </div>
            </div>
          </nav>
        </div>
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* Inner Sidebar - Only show on Campaigns view */}
          {activeView === 'campaigns' && (
            <aside className="hidden w-[220px] flex-col border-r border-slate-200 bg-white lg:flex shadow-sm z-0">
              <div className="flex-1 overflow-y-auto">
                <nav className="flex flex-col py-4">
                  <Link href="/advertiser-dashboard?view=campaigns" className="group flex items-center justify-between px-6 py-2.5 bg-teal-50 text-teal-600 border-r-2 border-teal-500">
                  ...
                  <div className="flex items-center px-6 py-2.5 text-[#757575] hover:bg-slate-50 cursor-pointer">
                    <div className="flex items-center gap-3">
                      <div className="h-5 w-5 flex items-center justify-center"><div className="w-3 h-3 border-2 border-current rounded-sm"></div></div>
                  ...
                  <div className="flex items-center px-6 py-2.5 text-[#757575] hover:bg-slate-50 cursor-pointer">
                    <div className="flex items-center gap-3">
                      <div className="h-5 w-5 flex items-center justify-center"><div className="w-3 h-3 bg-current rounded-sm"></div></div>
