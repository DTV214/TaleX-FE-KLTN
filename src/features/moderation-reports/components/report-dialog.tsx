            ...
            <div className="space-y-5">
              <label className="block">
                ...
                <select
                  ...
                  className={`h-11 w-full rounded-xl border border-white/10 bg-black/30 px-3 text-sm font-bold text-white outline-none transition focus:border-[#D4AF37]/70 disabled:cursor-not-allowed disabled:opacity-60 ${fixedReason ? "appearance-none" : ""}`}
                >
                  {reportReasonOptions
                    .filter((option) => !fixedReason || option.value === defaultReason)
                    .map((option) => (
              ...
              <label className="block rounded-xl border border-dashed border-white/15 bg-white/[0.03] p-4 transition hover:border-[#D4AF37]/40">
                ...
                <p className="mt-2 text-xs text-zinc-500">
                  Tối đa {MAX_FILES} file (Ảnh &lt; 5MB, Video &lt; 20MB)
                </p>
                {files.length > 0 && (
