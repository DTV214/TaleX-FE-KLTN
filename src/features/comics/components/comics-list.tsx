    ...
    <div className="relative min-h-screen overflow-hidden bg-[#12100d] pb-20 text-gray-100 antialiased">
      ...
      <div className="relative z-10 mx-auto w-full max-w-[1680px] space-y-12 px-4 pt-8 md:px-8">
        ...
        <section>
          ...
          {/* MỤC 5: TẤT CẢ TRUYỆN TRANH ĐỀ XUẤT (CUỘN VÔ CÙNG - API RECOMMENDATIONS FEED) */}
          <ComicRecommendationSection />
        </section>
      </div>
    </div>
  );
...
function ComicRecommendationSection() {
  const isSidebarOpen = usePublicSidebarStore((state) => state.isSidebarOpen);
  const loadMoreRef = useRef<HTMLDivElement | null>(null);

  // Calls GET /api/v1/recommendations/feed with pageType="COMICS"
  ...
  const {
    ...
    isFetchingNextPage,
    isLoading,
  } = useRecommendationFeedInfinite(12, "COMICS", {
  ...
  return (
    <section id="comics-all-recommendations" className="pt-4 space-y-6">
      ...
      {isLoading && comics.length === 0 ? (
        ...
        <EmptyState title="Chưa có dữ liệu truyện tranh đề xuất" />
      ) : (
        <>
          <div className={catalogGridClass}>
            {comics.map((comic, idx) => (
              <CatalogCard
                key={`${comic.seriesId || idx}-${idx}`}
                item={comic as unknown as SearchSeries}
              />
            ))}
          </div>
          ...
          >
            {hasNextPage && !isFetchingNextPage ? (
              <span className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2 text-xs font-bold text-white/38">
                Đang tải thêm truyện tranh đề xuất...
              </span>
            ) : null}
          </div>
        </>
