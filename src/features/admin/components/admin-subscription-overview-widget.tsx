...
import {
  adminStatisticsKeys,
  getSubscriptionOverviewStatistics,
  getSubscriptionDetailStatistics,
  type SubscriptionDetailItem,
...
interface AdminSubscriptionOverviewWidgetProps {
  startTime?: string;
  endTime?: string;
}

export function AdminSubscriptionOverviewWidget({
  startTime,
  endTime,
}: AdminSubscriptionOverviewWidgetProps) {
  ...
  const subscriptionOverviewQuery = useQuery({
    queryKey: adminStatisticsKeys.subscriptionOverview({
  ...
  const subscriptionDetailsQuery = useQuery({
    queryKey: adminStatisticsKeys.subscriptionDetails({
      startTime: finalStartTime,
      endTime: finalEndTime,
    }),
    queryFn: () =>
  ...
  const stats = subscriptionOverviewQuery.data;
  const details: SubscriptionDetailItem[] = subscriptionDetailsQuery.data ?? [];
  const currentGroupUnit = details[0]?.groupUnit;

  const handleRefetch = () => {
    void subscriptionOverviewQuery.refetch();
    void subscriptionDetailsQuery.refetch();
  };

  const isFetching =
    subscriptionOverviewQuery.isFetching || subscriptionDetailsQuery.isFetching;
    ...
    <div className="w-full rounded-2xl bg-white border border-gray-100 p-6 shadow-sm transition-all hover:shadow-md backoffice-dark:border-white/10 backoffice-dark:bg-white/[0.04]">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-100 pb-5 backoffice-dark:border-white/10">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 rounded-xl bg-amber-50 flex items-center justify-center backoffice-dark:bg-amber-500/20">
            <Crown className="h-5 w-5 text-amber-600 backoffice-dark:text-amber-300" />
          </div>
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="text-base font-bold text-gray-900 backoffice-dark:text-white">
                Thống Kê Doanh Thu Gói Premium (Subscription)
              </h3>
              <span className="inline-flex items-center gap-1 rounded-md bg-amber-50 px-2 py-0.5 text-[11px] font-semibold text-amber-700 backoffice-dark:bg-amber-950/50 backoffice-dark:text-amber-300 border border-amber-200 backoffice-dark:border-amber-800/40">
            ...
            <p className="text-xs text-gray-500 backoffice-dark:text-white/60 mt-0.5">
              Doanh thu từ các đơn hàng đăng ký gói Premium COMPLETED (SUBSCRIPTION)
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 shrink-0">
          <button
            ...
            title="Làm mới dữ liệu Premium"
          >
            <RefreshCw
              className={`w-3.5 h-3.5 ${isFetching ? "animate-spin text-amber-600" : ""}`}
            />
          </button>

          <Link
            href="/admin/subscriptions"
            className="inline-flex items-center gap-1.5 rounded-xl border border-amber-100 bg-amber-50 px-3.5 py-2 text-xs font-bold text-amber-600 transition hover:bg-amber-100 hover:text-amber-700 backoffice-dark:border-white/10 backoffice-dark:bg-white/[0.05] backoffice-dark:text-white/80 backoffice-dark:hover:bg-white/10 backoffice-dark:hover:text-white group"
          >
            <span>Quản lý Gói Premium</span>
            <ArrowRight className="h-3.5 w-3.5 text-amber-500 transition-transform group-hover:translate-x-0.5 group-hover:text-amber-700 backoffice-dark:text-white/60 backoffice-dark:group-hover:text-white" />
          </Link>
        </div>
      </div>
      ...
      {subscriptionOverviewQuery.isLoading && (
        <div className="py-10 flex items-center justify-center gap-2 text-xs font-medium text-gray-400">
          <span className="h-4 w-4 animate-spin rounded-full border-2 border-amber-600/25 border-t-amber-600" />
          Đang tải dữ liệu tổng quan gói Premium...
        </div>
      )}

      {/* Error state */}
      {subscriptionOverviewQuery.isError && (
        <div className="py-8 text-center">
          <p className="text-xs font-semibold text-red-500">
            Không thể tải dữ liệu thống kê gói Premium.
          </p>
          <Button
            type="button"
            onClick={handleRefetch}
            className="mt-3 h-8 bg-amber-600 px-3 text-xs font-semibold text-white hover:bg-amber-700 rounded-lg"
          >
            Thử lại
          </Button>
        </div>
      ...
      {/* Stats KPI Cards */}
      {!subscriptionOverviewQuery.isLoading && !subscriptionOverviewQuery.isError && (
        <>
          <div className="mt-5 grid grid-cols-1 sm:grid-cols-3 gap-5">
            ...
            <div className="rounded-xl bg-slate-50/70 p-5 border border-slate-100 transition-all hover:bg-slate-50 backoffice-dark:bg-white/[0.02] backoffice-dark:border-white/5">
              ...
              <p className="mt-1 text-[11px] text-gray-400">
                Doanh thu trước thuế từ gói Premium
              </p>
            </div>
            ...
            <div className="rounded-xl bg-slate-50/70 p-5 border border-slate-100 transition-all hover:bg-slate-50 backoffice-dark:bg-white/[0.02] backoffice-dark:border-white/5">
              ...
              <p className="mt-1 text-[11px] text-gray-400">
                Thuế giá trị gia tăng phát sinh (10%)
              </p>
            </div>
            ...
            <div className="rounded-xl bg-slate-50/70 p-5 border border-slate-100 transition-all hover:bg-slate-50 backoffice-dark:bg-white/[0.02] backoffice-dark:border-white/5">
              ...
              <p className="mt-1 text-[11px] text-gray-400">
                Doanh thu thuần (Gross - Thuế VAT)
              </p>
            </div>
          </div>
          ...
          <div className="mt-6 border-t border-gray-100 pt-5 backoffice-dark:border-white/10">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
              <div className="flex items-center gap-2">
                ...
                <h4 className="text-sm font-bold text-gray-900 backoffice-dark:text-white">
                  Biểu Đồ & Chi Tiết Doanh Thu Premium Theo Chuỗi Thời Gian
                </h4>
              </div>
              ...
              <div className="flex items-center gap-1 bg-gray-100 p-1 rounded-xl backoffice-dark:bg-white/10">
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-1 rounded-lg text-xs font-bold transition ${
                    viewMode === "chart"
                      ? "bg-white text-amber-700 shadow-sm backoffice-dark:bg-black/40 backoffice-dark:text-white"
            ...
            {subscriptionDetailsQuery.isLoading ? (
              ...
              <div className="flex h-56 items-center justify-center rounded-xl bg-slate-50 text-xs font-medium text-gray-400 backoffice-dark:bg-white/[0.02]">
                Không có dữ liệu chi tiết gói Premium trong khoảng thời gian này.
              </div>
            ) : viewMode === "chart" ? (
              /* Chart Mode */
              <div className="w-full h-72 pt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={details} margin={{ top: 10, right: 20, left: 10, bottom: 5 }}>
                    ...
                    <Bar dataKey="vatAmount" name="Tổng Thuế VAT" fill="#EF4444" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            ) : (
                  ...
                  <tbody className="divide-y divide-gray-100 backoffice-dark:divide-white/5">
                    {details.map((item) => (
                      <tr key={item.period} className="hover:bg-gray-50/50 transition">
                        ...
                        <td className="px-4 py-3 font-medium text-right text-rose-600">
